  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 md:px-6 md:py-16 space-y-16">
      ...
      <section className="space-y-6">
        <div className="space-y-3">
          ...
          <p className="text-muted-foreground text-base leading-relaxed max-w-lg">
            ...
            <code className="mx-1 rounded bg-muted px-1.5 py-0.5 font-mono text-xs text-foreground">npx shadcn add</code>
            でそのままプロジェクトに導入できます。
          </p>
        </div>
      ...
      <section className="space-y-4">
        <h2 className="text-sm font-semibold text-muted-foreground/60 tracking-wide uppercase">コンポーネント例</h2>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <GlowCard glowColor="purple" className="p-5 space-y-4">
            <div className="space-y-2">
              <p className="text-xs text-muted-foreground font-medium">Status Badge</p>
          ...
          <GlowCard glowColor="green" className="p-5 space-y-4">
            <div className="space-y-2">
              <p className="text-xs text-muted-foreground font-medium">Progress Ring</p>
              <div className="flex gap-4">
                <ProgressRing value={80} color="success" size={56} strokeWidth={6} />
                <ProgressRing value={45} color="warning" size={56} strokeWidth={6} />
                <ProgressRing value={30} color="danger" size={56} strokeWidth={6} />
              </div>
            </div>
            <div className="space-y-1.5">
          ...
          <GlowCard glowColor="orange" className="p-5 space-y-4">
            <div className="space-y-2">
              <p className="text-xs text-muted-foreground font-medium">Keyboard Shortcuts</p>
              <div className="space-y-1.5">
                {[["検索", ["⌘", "K"]], ["保存", ["⌘", "S"]], ["元に戻す", ["⌘", "Z"]]].map(([label, keys]) => (
            ...
            <div className="space-y-1.5">
              <p className="text-xs text-muted-foreground font-medium">Step Indicator</p>
              <StepIndicator
                currentStep={1}
                steps={[{ label: "入力" }, { label: "確認" }, { label: "完了" }]}
              />
            </div>
          </GlowCard>
        </div>
      ...
      <section>
        <div className="rounded-xl border border-border/60 bg-muted/20 p-5 space-y-4">
          ...
          <div className="rounded-lg border bg-muted/50 px-3 py-2 font-mono text-xs text-foreground/80 break-all">
            npx shadcn add https://ui.daigo-suhara.com/registry/[name].json
          </div>
